    ...
    getOne: baseProcedure.input(z.object({ id: z.string().uuid() })).query(async ({ input, ctx }) => {
        ...
        const viewerReactions = db.$with("viewer_reactions").as(
            db
                ...
                .from(videoReactions)
                .where(inArray(videoReactions.userId, userId ? [user.id] : [])),
        );
        ...
        const [existingVideo] = await db
            ...
            .select({
                ...
                likeCount: db.$count(
                    videoReactions,
                    and(eq(videoViews.videoId, videos.id), eq(videoReactions.type, "like")),
                ),
                dislikeCount: db.$count(
                    videoReactions,
                    and(eq(videoViews.videoId, videos.id), eq(videoReactions.type, "dislike")),
                ),
